...
def leer_arboles(nombre_archivo):
    with open(nombre_archivo, "r", encoding="UTF-8") as archivo:
        csv_file = csv.reader(archivo)
        headers = next(csv_file)
        lista_arboles = [{header: value for header,
            value in zip(headers, fila)} for fila in csv_file]
...
lista_tuplas_alturas_y_diametro_jacaranda = [(float(arbol['altura_tot']), float(
    arbol['diametro'])) for arbol in arboleda if arbol["nombre_com"] == "Jacarandá"]
...
def medidas_de_especies(especies, arboleda):
    ...
    for especie in especies:
        record = {especie: [(float(arbol['altura_tot']), float(
            arbol['diametro'])) for arbol in arboleda if arbol["nombre_com"] == especie]}
